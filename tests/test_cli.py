...
class TestCLI:
    ...
    def test_analyze_command_with_files(self):
        ...
        try:
            ...
            result = subprocess.run([
                sys.executable, '-m', 'firefly_analyzer.cli', 'analyze',
                '--cloud', cloud_path,
                '--iac', iac_path,
                '--output', output_path,
                '--verbose'
    ...
    def test_analyze_command_missing_files(self):
        ...
        result = subprocess.run([
            sys.executable, '-m', 'firefly_analyzer.cli', 'analyze',
            '--cloud', 'nonexistent.json',
            '--iac', 'nonexistent.json'
        ], capture_output=True, text=True)
        ...
        try:
            ...
            with patch('firefly_analyzer.cli.S3Uploader') as mock_uploader_class:
                ...
                result = subprocess.run([
                    ...
                    '--cloud', cloud_path,
                    '--iac', iac_path,
                    '--upload-s3', 'test-bucket',
                    '--s3-key', 'test-report.json',
                    '--localstack-endpoint', 'http://localhost:4566'
                ], capture_output=True, text=True)
                
                assert result.returncode == 0
                assert "Report uploaded to s3://test-bucket/test-report.json" in result.stdout
                
                # Verify S3 uploader was called
                mock_uploader.create_bucket_if_not_exists.assert_called_once_with('test-bucket')
    ...
    def test_download_command(self):
        ...
        with patch('firefly_analyzer.cli.S3Uploader') as mock_uploader_class:
            ...
            result = subprocess.run([
                sys.executable, '-m', 'firefly_analyzer.cli', 'download',
                '--bucket', 'test-bucket',
            ...
            # Verify S3 uploader was called
            mock_uploader.download_report.assert_called_once_with('test-bucket', 'test-report.json')
