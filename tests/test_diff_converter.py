...
class TestDiffConverter:
    ...
    def test_identical_resources(self):
        ...
        changelog = self.converter.convert_diff_to_changelog(
    ...
    def test_nested_property_change(self):
        ...
        cloud_resource = {
            "id": "ec2-1",
            "config": {"instance_type": "t3.large", "size": "100GB"},
        }

        iac_resource = {
            "id": "ec2-1",
            "config": {"instance_type": "t3.medium", "size": "100GB"},
        }
    ...
    def test_dictionary_item_added(self):
        ...
        iac_resource = {"id": "ec2-1", "tags": {"Environment": "production"}}
    ...
    def test_dictionary_item_removed(self):
        ...
        cloud_resource = {"id": "ec2-1", "tags": {"Environment": "production"}}
    ...
    def test_type_change(self):
        ...
        cloud_resource = {"id": "ec2-1", "config": {"size": 100}}

        iac_resource = {"id": "ec2-1", "config": {"size": "100GB"}}
    ...
    def test_array_differences(self):
        ...
        iac_resource = {"id": "ec2-1", "security_groups": ["sg-12345", "sg-67890"]}
        ...
        array_changes = [
            change for change in changelog if "security_groups" in change["KeyName"]
        ]
        assert len(array_changes) > 0

        # Check that we have changes for the new item (deepdiff breaks strings into chars)
        new_item_changes = [
            change for change in array_changes if "2[+]" in change["KeyName"]
        ]
