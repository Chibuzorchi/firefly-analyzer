...
class ResourceMatcher:
    ...
    def find_matching_iac_resource(
    ...
    def _primary_match(
        ...
        cloud_ids = [
            cloud_resource.get(field)
            for field in cloud_id_fields
            if cloud_resource.get(field)
        ]
    ...
    def _secondary_match(
        ...
        for iac_resource in iac_resources:
            ...
            if iac_type == cloud_type and iac_name == cloud_name:
                return iac_resource
    ...
    def _tertiary_match(
        ...
        if cloud_region:
            for iac_resource in iac_resources:
                ...
                if iac_name == cloud_name and iac_region == cloud_region:
                    return iac_resource
    ...
    def get_match_confidence(
        ...
        for cloud_field in cloud_id_fields:
            for iac_field in iac_id_fields:
                if (
                    cloud_resource.get(cloud_field)
                    and iac_resource.get(iac_field)
                    and cloud_resource[cloud_field] == iac_resource[iac_field]
                ):
                    return 1.0
