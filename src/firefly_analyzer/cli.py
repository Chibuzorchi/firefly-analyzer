...
@click.option(
    "--output", "-o", type=click.Path(), help="Output file path for analysis report"
)
@click.option("--upload-s3", "bucket_name", help="S3 bucket name to upload report")
@click.option("--s3-key", "s3_key", help="S3 object key for the report")
@click.option(
    "--localstack-endpoint", "endpoint_url", help="LocalStack S3 endpoint URL"
)
    ...
    try:
        ...
        if summary["modified"] > 0:
            ...
            for change_info in detailed_changes:
                ...
                click.echo(
                    f"  • {change['KeyName']}: '{change['IacValue']}' → '{change['CloudValue']}'"
                )
...
@click.option("--key", "-k", required=True, help="S3 object key")
@click.option(
    "--localstack-endpoint", "endpoint_url", help="LocalStack S3 endpoint URL"
)
@click.option(
    "--output", "-o", type=click.Path(), help="Output file path for downloaded report"
)
def download(bucket: str, key: str, endpoint_url: Optional[str], output: Optional[str]):
    """
