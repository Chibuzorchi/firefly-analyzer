...
class DiffConverter:
    ...
    def convert_diff_to_changelog(self, cloud_resource: Dict[str, Any], 
                                 iac_resource: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
    ...
    def compare_arrays_with_id_matching(self, cloud_array: List[Any], 
                                      iac_array: List[Any], 
                                      array_path: str) -> List[Dict[str, Any]]:
        """
        ...
        if (cloud_array and isinstance(cloud_array[0], dict) and 
            iac_array and isinstance(iac_array[0], dict)):
                            ...
                            if isinstance(changes_dict, dict):
                                for path, change in changes_dict.items():
                                    ...
                                    if isinstance(change, dict):
                                        changes.append({
                                            "KeyName": key_name,
                                            "CloudValue": change.get('new_value', change),
                                            "IacValue": change.get('old_value', change)
                                        })
                                    else:
                                        changes.append({
                                            "KeyName": key_name,
