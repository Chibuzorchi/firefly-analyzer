...
class CloudIacAnalyzer:
    ...
    def analyze(
        ...
        return {"analysis": analysis_results, "summary": summary}

    def _analyze_single_resource(
        ...
        matching_iac = self.matcher.find_matching_iac_resource(
    ...
    def _are_resources_identical(
        ...
        cloud_copy = {
            k: v
            for k, v in cloud_resource.items()
            if k not in ["id", "resource_id", "arn"]
        }
        iac_copy = {
            k: v
            for k, v in iac_resource.items()
            if k not in ["id", "resource_id", "arn"]
        }
    ...
    def _generate_summary(
        ...
        modified = sum(
            1 for result in analysis_results if result["State"] == "Modified"
        )
    ...
    def analyze_from_files(
        self, cloud_file: str, iac_file: str, output_file: Optional[str] = None
    ) -> Dict[str, Any]:
        """
            ...
            if result["State"] == "Modified" and result["ChangeLog"]:
                for change in result["ChangeLog"]:
                    detailed_changes.append(
                        {
                            "resource_id": result["CloudResourceItem"].get(
                                "id", "unknown"
                            ),
                            "resource_name": result["CloudResourceItem"].get(
                                "name", "unknown"
                            ),
                            "change": change,
                        }
